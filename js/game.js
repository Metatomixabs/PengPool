...
function _getCanvasZoom() {
  let el = C, z = 1;
  while (el) {
    const cs = parseFloat(getComputedStyle(el).zoom);
    if (cs && !isNaN(cs)) z *= cs;
    el = el.parentElement;
  }
  return z;
...
const WL = 55,
  ...
  WT = 76,
  WB = H - 63;
// Aim-line play-area bounds (ball-center space) derived from COLLISION_SHAPES at startup.
const _AIM_BOUNDS = (() => {
  const rails    = COLLISION_SHAPES.filter(s => s.type === 'rail');
  const laterals = rails.filter(s => s.h > s.w);
  const horizs   = rails.filter(s => s.w > s.h);
  const leftR  = laterals.reduce((a, b) => a.x < b.x ? a : b);
  const rightR = laterals.reduce((a, b) => a.x > b.x ? a : b);
  const topR   = horizs.reduce((a, b) => a.y < b.y ? a : b);
  const botR   = horizs.reduce((a, b) => a.y > b.y ? a : b);
  return {
    WL: leftR.x + leftR.w + R,
    WR: rightR.x - R,
    WT: topR.y + topR.h + R,
    WB: botR.y - R,
  };
...
function cueScreenPos() {
  const rect = C.getBoundingClientRect();
  const _z = _getCanvasZoom();
  const scaleX = (rect.width / _z) / W;
  const scaleY = (rect.height / _z) / H;
  return {
    x: rect.left / _z + cue.x * scaleX,
    y: rect.top / _z + cue.y * scaleY,
    sx: scaleX,
    sy: scaleY,
  };
}
...
function loop(ts) {
  ...
  if (DEBUG_CUSHIONS) {
    ...
    cx.lineWidth = 2;
    cx.beginPath();
    cx.moveTo(WL, WT); cx.lineTo(WL, WB); // left rail
    cx.moveTo(WR, WT); cx.lineTo(WR, WB); // right rail
    cx.moveTo(WL, WT); cx.lineTo(WR, WT); // top rail
    cx.moveTo(WL, WB); cx.lineTo(WR, WB); // bottom rail
    cx.stroke();
    // ── debugSegments (cian) ──
...
document.getElementById("pool").addEventListener("mousemove", (e) => {
  ...
  const _z = _getCanvasZoom();
  mouseX = (e.clientX / _z - rect.left / _z) / (rect.width / _z / W);
  mouseY = (e.clientY / _z - rect.top  / _z) / (rect.height / _z / H);
});
