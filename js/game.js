...
const R=11, FRIC=0.9875, MINS=0.1, MAXP=23, PR=21;
const WL=36, WR=W-37, WT=74, WB=H-55;
const DEBUG_CUSHIONS = false; // DEBUG: set to true to visualize cushion rails and gaps
const debugSegments = [
  [W/1.055-25,    H/5.05-25,   W/1.0165+25,    H/-20+25],   // L1
  [W/1.016-25,    H/3.85-25,   W/1.0165+25,    H/17+25],   // L1
  [W/8.95-25,    H/1.065-25,   W/-60+25,    H/1.065+25],   // L3
  [W/12.95-25,    H/1.14-25,   W/-23+25,    H/1.14+25],   // L3
  [W/1.92-25,    H/10-25, W/2.37+25,    H/10.15+25], // L7
  [W/1.755-25,    H/1.063-25, W/2.15+25,    H/1.065+25], // L5
  [W/2.05-25,    H/1.063-25, W/2.12+25,    H/1.065+25], // L6
  [W/1.87-25,    H/10-25, W/1.97+25,    H/10.2+25], // L7
  [W/1.053-25,    H/1.063-25,   W/1.053+25,    H/1.065+25],   // L8
  [W/1.015-25,    H/1.14-25,   W/1.0165+25,    H/1.14+25],   // L9
  [W/20-25,    H/10-25,   W/20+25,    H/10.2+25],   // L10
...
function loop(ts){
  ...
  if(DEBUG_CUSHIONS){
    const _cgTLh=30, _cgTLv=34, _cgTRh=30, _cgTRv=30, _cgBLh=29, _cgBLv=32, _cgBRh=28, _cgBRv=31, _mgT=33, _mgB=32, _offT=-3, _offB=-1.75;
    cx.save();
    // ── Rails AABB (rojo) ──
    cx.strokeStyle='rgba(255,0,0,0.8)';
    cx.lineWidth=2;
    cx.beginPath();
    cx.moveTo(WL, WT+_cgTLv); cx.lineTo(WL, WB-_cgBLv);        // left rail
    cx.moveTo(WR, WT+_cgTRv); cx.lineTo(WR, WB-_cgBRv);        // right rail
    cx.moveTo(WL+_cgTLh, WT); cx.lineTo(W/2+_offT-_mgT, WT);   // top-left segment
    cx.moveTo(W/2+_offT+_mgT, WT); cx.lineTo(WR-_cgTRh, WT);   // top-right segment
    cx.moveTo(WL+_cgBLh, WB); cx.lineTo(W/2+_offB-_mgB, WB);   // bottom-left segment
    cx.moveTo(W/2+_offB+_mgB, WB); cx.lineTo(WR-_cgBRh, WB);   // bottom-right segment
    cx.stroke();
    ...
    cx.lineWidth=2;
    cx.beginPath();
    for(const s of debugSegments){ cx.moveTo(s[0],s[1]); cx.lineTo(s[2],s[3]); }
    cx.stroke();
    // ── Corner deflection points (amarillo) — desactivado temporalmente ──
    // cx.fillStyle='rgba(255,255,0,0.9)';
    // const _dbgC=[
    //   {cx:W/2-_mg,cy:WT},{cx:W/2+_mg,cy:WT},
    //   {cx:W/2-_mg,cy:WB},{cx:W/2+_mg,cy:WB},
    //   {cx:WL+_cg, cy:WT},{cx:WL,     cy:WT+_cg},
    //   {cx:WR-_cg, cy:WT},{cx:WR,     cy:WT+_cg},
    //   {cx:WL+_cg, cy:WB},{cx:WL,     cy:WB-_cg},
    //   {cx:WR-_cg, cy:WB},{cx:WR,     cy:WB-_cg}
    // ];
    // for(const c of _dbgC){ cx.beginPath(); cx.arc(c.cx,c.cy,4,0,Math.PI*2); cx.fill(); }
    cx.restore();
  }
